...
data = None  # 'data/data/data_RECOLA_P16.npy'
...
if data is None:
    ...
    d = datasets.RECOLA(signals=[eda], subjects=subjects, annotations=[va, ar])
    print(d)
...
Y = [s.features.values for s in d.signals][0]
...
# Y = [data/np.std(data, axis=0) for data in Y]
...
if model is None:
    ...
    k1 = GPy.kern.RBF(Q[1], ARD=True, lengthscale=1) + GPy.kern.Bias(Q[1])
    ...
    k4 = GPy.kern.RBF(Q[2], ARD=False) + GPy.kern.Bias(Q[2])
    ...
    # deepGP
    m = deepgp.DeepGP([Q[0], Q[1], Q[2]], Y=Y, X=X,
                      kernels=[k1, k4],
                      num_inducing=N / 2,
    ...
    # m.layers[1].X.mean[:] = X[:].copy()
    # m.layers[1].X.mean.fix()

    # m.layers[1].X.variance[:] = 1e-6
    # m.layers[1].X.variance.fix()    
    # m.layers[2].Z[:] = X[:].copy()
    # m.layers[2].Z.fix()
