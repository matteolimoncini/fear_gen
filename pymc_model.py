...
with pm.Model() as sPPCA:
    ...
    x_pupil = pm.Normal('x_pupil', mu=Wpupil.dot(c.T), sigma=at.ones([D_pupil, N_pupil]),
                        shape=[D_pupil, N_pupil], observed=pupil_data)
    # eda
    x_eda = pm.Normal('x_eda', mu=Weda.dot(c.T), sigma=at.ones([D_eda, N_pupil]), shape=[D_eda, N_eda],
                      observed=eda_data)
...
with sPPCA:
    ...
    posterior_predictive = pm.sample_posterior_predictive(
        trace, random_seed=123, progressbar=False, var_names=['x_e'])
...
logging.info("Subject number: " + str(SUBJECT) + " Train Accuracy Pain Expectation: " + str(train_accuracy_exp))
