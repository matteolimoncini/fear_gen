...
hr_temp = np.concatenate([pd.read_csv('data/features/hr/' + str(x) + '.csv') for x in all_subject])
hr = np.concatenate([populate_array(x, hr_temp) for x in range(1, len(all_subject) + 1)])

pupil_temp = np.concatenate([pd.read_csv('data/features/pupil/' + str(x) + '.csv') for x in all_subject])
pupil = np.concatenate([populate_array(x, pupil_temp) for x in range(1, len(all_subject) + 1)])

eda_temp = np.concatenate([pd.read_csv('data/features/eda/' + str(x) + '.csv') for x in all_subject])
